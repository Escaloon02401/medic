...
var REPLIES=['Понял вас, спасибо.','Хорошо, давайте разберёмся.','Уточните симптомы подробнее?','Принято, изучу и отвечу.','Это важно учесть.'];
var PCARDS={
...
function initApp(){
  ...
  buildDash();buildDocs();buildAppts();buildChat();buildCard();goPage('dash');
...
function buildDash(){
  ...
  var stats=isDoc?[['&#128197;',DB.appts.length+'','Всего записей'],['&#9203;',pend+'','Ожидают подтверждения'],['&#128172;','8','Новых сообщений']]:[['&#128197;',active.length+'','Активных приёмов'],['&#9203;',pend+'','Ожидают'],['&#10084;','Норм.','Самочувствие']];
...
function confirmBook(){
  ...
  dbSave();closeM('bmodal');buildAppts();buildDash();goPage('appts');
  toast('Запись к '+SDOC.name+' отправлена — ожидайте подтверждения','green');
}

// CHAT
function buildChat(){
  var contacts=U.role==='patient'?[{id:'p1-1',name:'Анна Петрова',ini:'АП'},{id:'p1-2',name:'Михаил Соколов',ini:'МС'},{id:'p1-3',name:'Елена Воронова',ini:'ЕВ'}]:[{id:'p1-1',name:'Иван Иванов',ini:'ИИ'},{id:'p4-1',name:'Светлана Ким',ini:'СК'},{id:'p2-1',name:'Мария Сидорова',ini:'МС'}];
